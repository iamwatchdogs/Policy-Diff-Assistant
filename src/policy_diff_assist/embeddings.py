    ...
    if SentenceTransformer is not None:
        try:
            ...
            return EmbeddingBackend(
                name=model_name, dim=dim, model=model, fallback=False
            )
...
def embed_two_corpora(
    ...
    logger.info(
        "Batching the legacy & modern text together instead of individual ST calls for embedding."
    )
...
def embed_texts(
    backend: EmbeddingBackend, texts: list[str], batch_size: int = 2048
) -> np.ndarray:
    if not texts:
    ...
    try:
        ...
        logger.warning(
            "Embedding encode failed ({}); falling back to hash vectors", exc
        )
        return _hash_embeddings(texts, dim=backend.dim)


def embed_iterable(
    backend: EmbeddingBackend, texts: Iterable[str], batch_size: int = 64
) -> np.ndarray:
